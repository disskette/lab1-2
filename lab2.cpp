...
long linear_search(Partner a[], long size, const char* name, long last_index)
{
...
long binary_search(Partner a[], long start, long end, const char* name)
{
...
int main(int argc, char* argv[]) //Все имена файлов csv с входными данными передаются при запуске программы
{
    ...
    for (int i=1; i<argc; i++) // Пробегаюсь по всем именам CSV
    {
        ...
        {
            size_t len = strlen(p1[size / 2].getOrg()); // Делаю копию строки, иначе будет косяк по адресу после сортировки
            char* searchKey = new char[len + 1]; // Ключ, по которому буду искать
            ...
            if (searchCheck(multimapArr, linearArr, binaryArr,redblackArr, hashArr, found_size)==true) // Поиски сошлись
            {
                ...
                std::cout << "RedBlack: " << redblackSec << "нс;  " << "HashTable: " << hashSec<< "нс" << "\n" << std::endl;
            }
